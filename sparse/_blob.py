...
class Blob(object):
    ...
    as many Blobs do not need the gradients to be computed. <------- ------- ------- WHY???
    """
    ...
    def init_data(self, shape, dtype, setdata=True):
        ...
        if not(self.has_data() and self._data.shape == shape and \
           self._data.dtype == dtype):
            self._data = np.empty(shape, dtype) #Returns a new array of given shape and type, without initializing entries
    ...
    def init_diff(self, setzero=True):
        ...
        if self.has_diff():
            ...
            self._diff = np.zeros_like(self._data) #Returns an array of zeros with the same shape and type as a given array
        return self.diff()
