...
from sparse._blob import Blob

class SparseError(Exception):
    """NOOOOOOO! I need caffeine!
    
    Yes, this is the basic error type under sparse.
    """
    pass


class InvalidLayerError(SparseError):
    """The error when an invalid spec is passed to a layer."""
    pass


class InvalidNetError(SparseError):
...
class DataLayer(Layer):
    ...
    def backward(self, bottom, top, propagate_down):
        ...
        """
        raise SparseError('You should not reach this.')
...
class Solver(object):
    ...
    def solve(self, net, previous_net=None):
        ...
        Input:
            net: the sparse network to run optimization on.
...
class Net(object):
    ...
    def __init__(self, name=None, input_image=None):
        """Initialize a net.
        Input:
    ...
    def add_layer(self, layer, needs=None, provides=None, group=-1):
        ...
        Args:
            layer: a sparse.base.Layer instance.
