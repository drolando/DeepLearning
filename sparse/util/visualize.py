...
def draw_net(sparse_net, ext='png'):
    """Draws a sparse net and returns the image string encoded using the given
    extension.
    
    Input:
        sparse_net: a sparse net.
        ext: the image extension. Default 'png'.
    """
    pydot_graph = pydot.Dot(graph_type='digraph')
    pydot_nodes = {}
    for name, layer in sparse_net.layers.iteritems():
        pydot_nodes[name] = pydot.Node(
            '{%s|%s}' % (name, layer.__class__.__name__), **LAYER_STYLE)
    for name, blob in sparse_net.blobs.iteritems():
        if blob.has_data():
    ...
    '''for name in pydot_nodes:
        print pydot_nodes[name]
        pydot_graph.add_node(pydot_nodes[name])'''
    # only write explicit edges
    for layername, blobnames in sparse_net.provides.iteritems():
        if layername.startswith(base.SPARSE_PREFIX):
            continue
        for blobname in blobnames:
            pydot_graph.add_edge(
                pydot.Edge(pydot_nodes[layername], pydot_nodes[blobname]))
    for layername, blobnames in sparse_net.needs.iteritems():
        if layername.startswith(base.SPARSE_PREFIX):
            continue
        for blobname in blobnames:
            pydot_graph.add_edge(
...
def draw_net_to_file(sparse_net, filename):
    """Draws a sparse net, and saves it to file using the format given as the
    file extension.
    """
    ext = os.path.splitext(filename)[-1][1:]
    print filename
    with open(filename, 'w') as fid:
        fid.write(draw_net(sparse_net, ext))
