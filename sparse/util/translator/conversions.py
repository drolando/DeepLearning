"""Conversions converts data from the cuda convnet convention to the sparse
convention."""
import numpy as np



def imgs_cudaconv_to_sparse(imgs, size, channels, out=None):
...
def img_cudaconv_to_sparse(img, size, channels, out=None):
    """See imgs_cudaconv_to_sparse for details. The only difference is that
    this function deals with a single image of shape (channels, size, size).
    """
    if out is None:
        out = np.empty((size, size, channels), img.dtype)
    return imgs_cudaconv_to_sparse(img[np.newaxis, :], size, channels,
