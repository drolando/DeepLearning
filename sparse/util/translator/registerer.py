...
# for the corresponding sparse layer.
...
def translate_layer(cuda_layer, output_shapes):
    """Translates a cuda layer to a sparse layer. The function will return
    False if the input layer is a data layer, in which no sparse layer needs to
    be inserted.
    ...
    Input:
        ...
        output_shapes: a dictionary keeping the output shapes of all the 
            layers.
    Output:
        sparse_layer: the corresponding sparse layer, or False if the input is a
            data layer.
    """
...
def translate_cuda_network(cuda_layers, output_shapes):
    """Translates a list of cuda layers to a sparse net.
    ...
    Input:
        ...
        output_shapes: a dictionary that contains the specification on the
            ...
            sparse convention, not the cuda convnet convention.
    """
    sparse_net = base.Net()
    for cuda_layer in cuda_layers:
        sparse_layer = translate_layer(cuda_layer, output_shapes)
        if not sparse_layer:
        ...
        sparse_net.add_layers(sparse_layer, needs=needs, provides=provide)
    sparse_net.finish()
    return sparse_net
