...
class StochasticSolver(base.Solver):
    ...
    def compute_update_value(self):
        ...
        as [param.diff() for param in sparse_net.params()], and the algorithm
    ...
    def snapshot(self, is_final=False):
        ...
        try:
            ...
            pass
        self._sparse_net.save(
            os.path.join(subfolder, self._sparse_net.name + '.net'))
...
class SGDSolver(StochasticSolver):
    ...
    def compute_update_value(self):
        ...
        if self.spec['momentum'] > 0:
            ...
            for param in self._sparse_net.params():
                #if param.diff() == None:
                #    param.init_diff()
                diff = param.diff()
                diff *= learningrate
    ...
    def iter_callback(self, loss):
        ...
        if self.spec['asgd'] and self._iter_idx >= self.spec['asgd_skip']:
            ...
            for asgd, param in zip(self._asgd, self._sparse_net.params()):
...
class AdagradSolver(StochasticSolver):
    ...
    def compute_update_value(self):
        ...
        learning rate.
        """
        for param, accum in zip(self._sparse_net.params(), self._accum):
            diff = param.diff()
